...
- **`dashboard-server.py` + `iniciar-dashboard.sh` (Linux/Mac) / `iniciar-dashboard.bat` (Windows)** — mini-servidor local (Python padrão, sem dependências). O usuário dá duplo clique no `.sh/.bat` → abre `http://localhost:8765` com o painel completo: editar, excluir e arrastar cards salvam direto no banco.
...
1. Copie `references/dashboard-server.py` e `references/iniciar-dashboard.sh` desta skill para a raiz da pasta conectada.
...
4. Diga ao usuário: "duplo clique em `iniciar-dashboard.sh` abre o painel com o banco conectado" (requer Python instalado no Linux/Mac/Windows; sem ele, o dashboard.html funciona no modo arquivo).
...
```
   ...
   - `/redesenhar` → `status='redesenhado'`
   - `/publicar` → `status='publicado'`, `urlNova`
